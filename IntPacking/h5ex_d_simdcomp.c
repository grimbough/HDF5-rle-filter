...
#define FILE            "h5ex_d_sdc.h5"
#define DATASET         "DS1"
#define DIM0            32
#define DIM1            16
#define CHUNK0          4
#define CHUNK1          8
#define H5Z_FILTER_SDC        30010
...
main (void)
{
    ...
    const unsigned int cd_values[2] = {1, 6};     /* number of bits to use */
    ...
    status = H5Pset_filter (dcpl_id, H5Z_FILTER_SDC, H5Z_FLAG_MANDATORY, (size_t)2, cd_values);
    if (status < 0) goto done;
     ...
     */
    avail = H5Zfilter_avail(H5Z_FILTER_SDC);
    if (avail) {
        status = H5Zget_filter_info (H5Z_FILTER_SDC, &filter_config);
        if ( (filter_config & H5Z_FILTER_CONFIG_ENCODE_ENABLED) && 
                (filter_config & H5Z_FILTER_CONFIG_DECODE_ENABLED) ) 
            printf ("rle filter is available for encoding and decoding.\n");
    }     
     ...
     * Create the dataset.
     */
    printf ("....Writing rle compressed data ................\n");
     ...
     */
    printf ("....Reading rle compressed data ................\n");
    ...
    avail = H5Zfilter_avail(H5Z_FILTER_SDC);
    if (avail)  
        printf ("RLE filter is available now since H5Dread triggered loading of the filter.\n");
