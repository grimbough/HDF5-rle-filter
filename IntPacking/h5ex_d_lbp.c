...
main (void)
{
    ...
    status = H5Pset_filter (dcpl_id, H5Z_FILTER_LBP, H5Z_FLAG_MANDATORY, (size_t)2, cd_values);
    if (status < 0) goto done;
     ...
     */
    avail = H5Zfilter_avail(H5Z_FILTER_LBP);
    if (avail) {
        status = H5Zget_filter_info (H5Z_FILTER_LBP, &filter_config);
