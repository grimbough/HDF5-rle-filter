...
main (void)
{
    ...
    status = H5Pset_filter (dcpl_id, H5Z_FILTER_TRLE, H5Z_FLAG_MANDATORY, (size_t)0, cd_values);
    if (status < 0) goto done;
     ...
     */
    avail = H5Zfilter_avail(H5Z_FILTER_TRLE);
    if (avail) {
        status = H5Zget_filter_info (H5Z_FILTER_TRLE, &filter_config);
