...
main (void)
{
    ...
    status = H5Pset_filter (dcpl_id, H5Z_FILTER_RLE8, H5Z_FLAG_MANDATORY, (size_t)0, cd_values);
    if (status < 0) goto done;
     ...
     */
    avail = H5Zfilter_avail(H5Z_FILTER_RLE8);
    if (avail) {
        status = H5Zget_filter_info (H5Z_FILTER_RLE8, &filter_config);
    ...
    avail = H5Zfilter_avail(H5Z_FILTER_RLE8);
    if (avail)  
        printf ("RLE8 filter is available now since H5Dread triggered loading of the filter.\n");
